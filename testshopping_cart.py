...
class TestShoppingCart:
    ...
    def test_remove_item_exact(self):
        ...
        assert not self.cart.view_cart()
    ...
    def test_pay_items_partial(self):
        ...
        total = self.cart.pay_items([
            {'name': 'apple', 'price': 2.0, 'quantity': 3}
        ])
        assert total == 6.0
    ...
    def test_pay_items_exact(self):
        ...
        total = self.cart.pay_items([
            {'name': 'orange', 'price': 3.0, 'quantity': 2}
        ])
        assert total == 6.0
        assert not self.cart.view_cart()
    ...
    def test_update_item_name(self):
        ...
        assert self.cart.view_cart() == [
            {'name': 'baguette', 'quantity': 1, 'price': 2.0}
        ]
    ...
    def test_update_item_quantity(self):
        ...
        assert self.cart.view_cart() == [
            {'name': 'soda', 'quantity': 5, 'price': 1.0}
        ]
    ...
    def test_update_item_price(self):
        ...
        assert self.cart.view_cart() == [
            {'name': 'chips', 'quantity': 3, 'price': 2.0}
        ]
