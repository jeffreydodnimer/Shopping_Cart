...
class TestShoppingCart:
    ...
    def test_cart_operations(self):
        ...
        # Test exceptions and payment
        with pytest.raises(ValueError):
            self.cart.remove_item("egg", 1, 1.0)
        total = self.cart.pay_items([{'name': 'apple', 'price': 1.5, 'quantity': 2}])
        assert total == 3.0
        assert self.cart.view_cart() == [{'name': 'apple', 'quantity': 0, 'price': 1.5}]
        ...
        assert self.cart.view_cart() == [{'name': 'green apple', 'quantity': 3, 'price': 2.0}]
        
        # Test item not found for updates
        for method in [self.cart.update_item_name, self.cart.update_item_quantity, self.cart.update_item_price]:
            with pytest.raises(ValueError):
